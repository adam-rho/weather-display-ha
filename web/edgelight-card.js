...
import {defaults,render,validate,hex,glow,names,wetColor,sample,conditionBucket,ledFor,origins,normalize} from './display-model.js';
...
export class EdgelightDisplayCard extends LitElement {
  ...
  set hass(hass){
    ...
    if(state?.config&&!validate(state.config)&&Number.isInteger(state.revision)){
      ...
      if(!this.dirty)this.draft=normalize(clone(state.config));
    }
  ...
  render(){
    ...
    return html`<link rel="stylesheet" href=${new URL('./edgelight.css',import.meta.url)}><article>
      <header><div><h2>Edgelight</h2></div><span class="connection ${this.online?'online':''}">${this.online?'Display online':'Display offline'}</span></header>
